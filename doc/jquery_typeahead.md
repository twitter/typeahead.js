...
* `typeahead:rendered` – Fired when suggestions are rendered for a dataset. The
...
* `typeahead:autocompleted` – Fired when a autocompletion occurs. The
...
* `typeahead:cursorchanged` – Fired when the results container cursor moves. The
  event handler will be invoked with 2 arguments: the jQuery event object and
